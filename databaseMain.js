...
static dağıtıcıOn() {
    Array.prototype.shuffle = function () {
      ...
      const Tokens = acar.databaseTokenler
      class CLIENT {
        constructor(token) {
          this.token = token;
      ...
      for (let c in clientObject) {
        ...
        c.on("ready", async () => {
            c.user.setPresence({ activity: { name: acar.botDurum }, status: 'online' });
        ...
        c.on("message", async message => {
            ...
            if(command === "kur" || command === "kurulum" || command === "backup" || command === "setup") {
              if (!args[0] || isNaN(args[0])) return;
                ...
                setTimeout(() => {
                  Database.findOne({guildID: acar.sunucuID, roleID: args[0]}, async (err, roleData) => {
                    if (!roleData) return;
                    message.react("✅");
...
static fetchCmdDatabase() {
 ...
 static async dMain() {
        databaseclient.on("ready", async () => {
            databaseclient.user.setPresence({ activity: { name: acar.botDurum }, status: acar.botStatu });
            let botVoiceChannel = databaseclient.channels.cache.get(acar.botSesKanali);
...
function guvenli(kisiID) {
    let uye = databaseclient.guilds.cache.get(acar.sunucuID).members.cache.get(kisiID);
    let guvenliler = acar.whitelist || [];
    if (!uye || uye.id === databaseclient.user.id || uye.id === acar.botSahip || uye.id === uye.guild.owner.id || guvenliler.some(g => uye.id === g.slice(1) || uye.roles.cache.has(g.slice(1)))) return true
    else return false;
  };
  ...
  function cezalandir(kisiID, tur) {
    ...
    if (newMember.roles.cache.size > oldMember.roles.cache.size) {
      ...
      if (yetkiPermleri.some(p => oldMember.hasPermission(p) && newMember.hasPermission(p))) {
        ...
        let logKanali = databaseclient.channels.cache.get(acar.logKanalı);
        if (logKanali) { logKanali.send(new MessageEmbed().setColor("#2F3236").setTitle('Sağtık Yetki İşlemi Yapıldı!').setDescription(`${newMember} (__${newMember.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından Sağtık Yetki İşlemi Yapıldı! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(acar.botDurum)).catch(); } else { newMember.guild.owner.send(new MessageEmbed().setColor("#2f3236").setTitle('Sağtık Yetki İşlemi Yapıldı!').setDescription(`${newMember} (__${newMember.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından Sağtık Yetki İşlemi Yapıldı! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(acar.botDurum)).catch(err => {}); 
  ...
  databaseclient.on("roleCreate", async role => {
    ...
    if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) ) return;
    role.delete({ reason: "ACAR Guard Rol Koruma" });
   cezalandir(entry.executor.id, "ban");
    let logKanali = databaseclient.channels.cache.get(acar.logKanalı);
    if (logKanali) { logKanali.send(new MessageEmbed().setColor("#2f3236").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (__${entry.executor.id}__) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter(acar.botDurum)).catch(); } else { role.guild.owner.send(new MessageEmbed().setColor("#2f3236").setTitle('Rol Oluşturuldu!').setDescription(`${entry.executor} (__${entry.executor.id}__) tarafından bir rol oluşturuldu! Oluşturan kişi jaile atıldı ve rol silindi.`).setFooter(acar.botDurum)).catch(err => {}); };
  });
  ...
  function ytKapat(guildID) {
    ...
    static databaseBackup() {
        let guild = databaseclient.guilds.cache.get(acar.sunucuID);
        if (guild) {
          guild.roles.cache.filter(r => r.name !== "@everyone" && !r.managed).forEach(role => {
            ...
            Database.findOne({guildID: acar.sunucuID, roleID: role.id}, async (err, savedRole) => {
              if (!savedRole) {
                let newRoleSchema = new Database({
                  _id: new mongoose.Types.ObjectId(),
                  guildID: acar.sunucuID,
                  roleID: role.id,
                  name: role.name,
                  color: role.hexColor,
                  hoist: role.hoist,
          ...
          Database.find({guildID: acar.sunucuID}).sort().exec((err, roles) => {
          ...
          console.log(`[ ~ ACAR ~ ] Roller veritabanına yedeklenip veritabanına başarıyla işlendi.`);
        };
    }
    static async On() {
        databaseclient.login(acar.databaseToken).catch(acar => console.log("[ ACAR Database ] Discord API tarafından token'i Doğrulanamadı!"))  
    }
}
